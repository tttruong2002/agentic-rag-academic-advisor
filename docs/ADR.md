...
## ADR-006: Dùng `.agents/skills/` chuẩn SKILL.md quốc tế
...
**Quyết định**: Dùng `.agents/skills/` thay vì `.agent/skills/` (cũ) hay `.cursor/rules/`.
...
## ADR-007: Chuyển tài liệu từ `.context/` sang `docs/`

**Ngày**: 12/05/2026

**Trạng thái**: Accepted

**Bối cảnh**: Thư mục `.context/` là convention riêng, không phải chuẩn quốc tế. Cần thư mục tài liệu mà mọi AI tool và developer đều nhận ra.

**Quyết định**: Chuyển toàn bộ tài liệu sang `docs/` với tên file chuẩn hơn (`SUMMARY.md` → `OVERVIEW.md`, `DECISIONS.md` → `ADR.md`).

**Lý do**:
- `docs/` là convention phổ biến nhất trên GitHub
- Tên file chuẩn hơn (ADR là thuật ngữ ngành, Overview rõ ràng hơn Summary)
- Cross-AI compatibility: GitHub Copilot, Cursor, Claude đều tự động nhận `docs/`
